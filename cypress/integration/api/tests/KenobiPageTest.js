...
describe("Kenobi Test Flow", () => {
    ...
    it('Get kenobi chat list', () => {
        doGetKenobiChatList(authKey, app_id).then((response) => {
            cy.log("Get kenobi chat list Response", response.body)
            expect(response.status).to.eq(200)
        })
    })
    it('Create kenobi chat', () => {
        doCreateKenobiChat(authKey, app_id).then((response) => {
            cy.log("Create Kenobi Chat Response", response.body)
            expect(response.status).to.eq(201)
        })
    })
    it('Clear Kenobi Chat', () => {
        doClearKenobiChat(authKey, app_id).then((response) => {
            cy.log("Clear Kenobi Chat Response", response.body)
            expect(response.status).to.eq(201)
        })
    })
